...
for i in range(trajectory_num):
    ...
    with open(data_root + f"/images/traj_{i}.pkl", 'rb') as file:
    ...
    with h5py.File(new_file_path + '.hdf5', 'w', rdcc_nbytes=1024 ** 2 * 2) as root:
        ...
        #                         chunks=(1, 480, 640, 3), )
        proprioception = obs.create_dataset('proprioception', (max_timesteps, 28))
        flow = obs.create_dataset('flow', (max_timesteps, 128))
        dynamo = obs.create_dataset('dynamo', (max_timesteps, 128))
        action = root.create_dataset('actions', (max_timesteps, 28))
