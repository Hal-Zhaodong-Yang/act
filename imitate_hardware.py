...
def main(args):
    ...
    from constants import HARDWARE_TASK_CONFIGS
    task_config = HARDWARE_TASK_CONFIGS[task_name]
    ...
    best_ckpt_info = train_bc(train_dataloader, config)
...
def forward_pass(data, policy):
    image_data, qpos_data, flow_data, action_data, is_pad = data
    image_data = image_data.cuda()
    qpos_data = qpos_data.cuda()
    flow_data = flow_data.cuda()
    action_data = action_data.cuda()
    is_pad = is_pad.cuda()
    return policy(qpos_data, image_data, flow_data, action_data, is_pad) # TODO remove None
...
def train_bc(train_dataloader, config):
    ...
    for epoch in tqdm(range(num_epochs)):
        ...
        with torch.inference_mode():
            policy.eval()
            epoch_dicts = []
            # TODO: For now use the training set to evaluate, ACT originally uses val set
...
if __name__ == '__main__':
    ...
    parser.add_argument('--object_obs', action='store', type=str, help='type of object observation', required=True)
